// Medium benchmark: Bounded MPMC queue with component stress testing
// Validates coordination across two Condvars under deterministic stress configs.
...
fn bench_bounded_mpmc_deloxide_with_component_stress(c: &mut Criterion) {
    for &burst in &BURST_SIZES {
        c.bench_function(
            &format!(
                "bounded_mpmc_deloxide_with_component_stress_{}msgs",
                burst
            ),
            |b| {
...
criterion_group!(
    ...
    targets = bench_bounded_mpmc_deloxide_with_component_stress,
);
