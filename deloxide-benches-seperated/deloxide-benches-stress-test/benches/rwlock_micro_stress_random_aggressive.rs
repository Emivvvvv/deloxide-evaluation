...
fn bench_read_stress_aggressive(c: &mut Criterion) {
    start_random_stress_with(StressConfig::aggressive());
    let l = Arc::new(RwLock::new(0usize));
    c.bench_function("rwlock_read_stress_random_aggressive", |b| {
        b.iter(|| {
            let _g = l.read();
        });
    });
...
fn bench_write_stress_aggressive(c: &mut Criterion) {
    start_random_stress_with(StressConfig::aggressive());
    let l = Arc::new(RwLock::new(0usize));
    c.bench_function("rwlock_write_stress_random_aggressive", |b| {
        b.iter(|| {
            let mut g = l.write();
            *g += 1;
        });
...
criterion_group!(
    ...
    targets = 
        bench_read_stress_aggressive,
        bench_write_stress_aggressive
);
